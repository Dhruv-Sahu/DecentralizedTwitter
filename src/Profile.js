...
// const client = ipfsHttpClient('https://dhruv.infura-ipfs.io/')
const projectId = '2IBM9kLv4JwpxVLc85YEbo8n3Gj';
const projectSecret = '0c78915b765853d5b30956562f8214ea';
...
//     protocol: 'https',
//     headers: {
//         authorization: '0c78915b765853d5b30956562f8214ea' // Replace with your API key secret
//     }
// });
...
const App = ({ contract }) => {
    ...
    const loadMyNFTs = async () => {
        console.log("LoadMyNfts button hit")
        // Get users nft ids
    ...
    const uploadToIPFS = async (event) => {
        ...
        if (typeof file !== 'undefined') {
            try {
                ...
                setAvatar(`https://dhruv.infura-ipfs.io/ipfs/${result.path}`)
    ...
    // QmSVdohKwqQYkR2aFMEAo5ZyrMUUuQ6k6sBtJzsfUxhqjT

    const mintProfile = async (event) => {
        console.log("Mint NFT profile Button clicked");
        if (!avatar || !username) return
        try {
            const result = await client.add(JSON.stringify({ avatar, username }))
            setLoading(true)
            await (await contract.mint(`https://dhruv.infura-ipfs.io/ipfs/${result.path}`)).wait()
            loadMyNFTs()
        } catch (error) {
